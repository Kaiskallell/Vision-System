...
PickConveyor::PickConveyor(const fs::path& cameraConfigPath,
...
PickConveyor::~PickConveyor() { cv::destroyAllWindows(); }

void PickConveyor::detectPosesWithNetworks(std::vector<VSPose>& posesInRobotCoordinates, std::vector<int>& classIds)
{
	...
	YoloDLCResults keyPointsAndClasses =
	    m_yoloDlcPipeline->run(networkInputImg, m_maxAmountOfProductsPerImg, YoloDLC::kAllClasses);
	...
	if (m_depthOnlyOnDetections)
	{
		// do a depth calculation only on the area where a dettection was made
		m_depthOnlyOnDetection->run(
		    m_frame.colorImage, m_frame.secondBgrImage, keyPointsAndClasses.m_rects, m_frame.depthImage);
	}
...
void PickConveyor::estimatePose(YoloDLCResults& keyPointsAndClasses,
...
void PickConveyor::run()
{
	...
	while (!visionsystemsignals::shutdownProgam && !m_dbFacade->getShutDownProcesses())
	{
		...
		if (!ret || !frameUpdated)
		{
			continue;  // error with communication or no trigger made
		}
